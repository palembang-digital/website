import React from "react";

import s from "./Visi.module.scss";

const Misi = () => {
  return (
    <div id={s["misi"]}>
      <ol>
        <li>
          Menjadikan talent-talent lokal Sumsel untuk memiliki jiwa yang
          kolaboratif dan inovatif lewat pembelajaran / workshop yang diadakan
        </li>
        <li>
          Membuat ekosistem digital yang terus beregenerasi untuk
          keberlangsungan talent lokal Sumsel
        </li>
        <li>
          Meningkatkan skill dalam hal wawasan dan pengetahuan di bidang digital
          seperti software development, infrastruktur IT dan lain sebagainya
          yang berkaitan dengan dunia digital
        </li>
      </ol>
    </div>
  );
};

export default Misi;
